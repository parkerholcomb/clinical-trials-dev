...
def _get_us_locations(protocol_features: pd.DataFrame, country: list = ["United States"]) -> pd.DataFrame:
    """
        Get locations of Phase 2, 3, 4
    """
    df = protocol_features
    df = df[df['_location_count_z'] < 3] # filter < 3 sigma 
    df = df[df['_location_count'] > 0] 
    df = df.sort_values("_location_count")
    # columns_to_join = [] # revist this
...
def _get_protocol_feature(protocol: pd.DataFrame) -> pd.DataFrame:
    ...
    feature["_phase"] = protocol["Phase"].dropna().apply(lambda x: x[0]) # come back to this
