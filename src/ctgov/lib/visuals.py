...
def draw_org_fragmentation(protocol_features):
    ...
    def _draw_top_treemap(top_n=500):
        px.treemap(
            org_counts[0:top_n],
            path=["_org_class", "_org_name"],
            values="study_count",
            height=700,
            title=f"Org Fragmentation top {top_n} of {_org_count} orgs shown | grouped by class, scaled by study count | n={_study_count} studies ",
        ).show(renderer="notebook")
